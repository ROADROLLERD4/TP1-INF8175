...
Pacman agents (in searchAgents.py).conda create --name devoir1 python=2.8
"""
...
def depthFirstSearch(problem:SearchProblem)->List[Direction]:
    ...
    while not listFringe.isEmpty():
        ...
        #     parent[newState[0]] = (currentState[0], newState[1])
        ...
        if problem.isGoalState(currentState[0]):
            ...
            for node in successorsList:
                if node[0] not in listExplored:
                    ...
                    parent[node[0]] = (currentState[0], node[1])
    return []
...
def breadthFirstSearch(problem:SearchProblem)->List[Direction]:
    ...
    parent = {}
    initialState = problem.getStartState() 
    listFringe = util.Queue() #changement de Stack à Queue pour BFS
    listExplored = []
    ...
    while not listFringe.isEmpty():
        ...
        if problem.isGoalState(currentState[0]):
            ...
            successorsList = problem.getSuccessors(currentState[0])
            for node in successorsList:
                if node[0] not in listExplored and node[0] not in parent:
                    listFringe.push(node)
                    parent[node[0]] = (currentState[0], node[1])
    return []
